...
/* O */ {245, 244, 243, 242, 241, 240, 239, 238, 220, 221, 222, 223, 224, 225, 226, 227, 228, 229, 211, 210, 209, 179, 180, 181, 167, 166, 165, 131, 132, 133, 115, 114, 113, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 202, 201, 200, 188, 189, 190, 158, 157, 156, 140, 141, 142, 106, 105, 104, 53, 52, 51, 50, 49, 48, 47, 46},
...
byte heartAnimation[] = {50, 49, 83, 82, 81, 80, 79, 78, 114, 113, 112, 111, 110, 109, 108, 107, 106, 105, 143, 142, 141, 140, 139, 138, 137, 136, 135, 134, 133, 132, 131, 130, 169, 168, 167, 166, 165, 164, 163, 162, 161, 160, 159, 158, 157, 156, 155, 154, 184, 185, 183, 186, 182, 187, 181, 188, 180, 189, 179, 190, 178, 191, 177, 192, 176, 193, 206, 205, 207, 204, 208, 203, 209, 202, 210, 201, 211, 200, 212, 199, 213, 198, 214, 197, 223, 226, 222, 227, 221, 228, 220, 229, 219, 230, 218, 231, 217, 232, 244, 239, 245, 238, 246, 237, 247, 236};
...
// only works in idle mode to save computational power for other tasks. 
  if (mode == "idle"){
      ...
      if (Firebase.getBool(firebaseData, treeId + "/intruderDetection/active/")) { 
...
void listeningForMessages(){
 ...
 void handleBallShakeAnimation() {
    ...
    strip.clear();
    for (int i =0; i < 106; i++){
          strip.setPixelColor(heartAnimation[i], 0xFF0000);
          strip.show();
    }
    delay(2000);
    //set mode back to idle
